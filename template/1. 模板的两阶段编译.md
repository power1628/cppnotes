...
当在调用func函数的时候，需要对TemplParam和CallParam类型实例化，此时涉及到类型的推导和自动转换。
...
2. 当CallParam被声明为值（value）时，只会发生**基本（trivial）类型**的衰变转换(decay)。decay的规则可以参考`std::decay`。
...
关于`std::decay<T>`的衰变规则:
	...
	1. 转换将移除T类型的`cv限定符(const 和 volatile)`
